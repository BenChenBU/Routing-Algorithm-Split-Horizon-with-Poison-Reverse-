    ...
    void rtupdate(Packet rcvdpkt) {
       ...
       // initializing local variables
        ...
        for(int i=0; i< 4; i++){
            ...
            x = mincost[i] + lkcost[source];  // add the new value from the minCost array to the path to the exist mincost
            
            if(x != INFINITY && x != costs[i][source]){ // if the node is not unreachable and it is not equal to itself add it
